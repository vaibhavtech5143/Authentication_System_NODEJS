...
const resend = new Resend(config.SENDGRID_API_KEY);
    ...
    (err, existingUser) => {
      if (existingUser) {
        ...
        resend.emails.send(msg)
          .then(() => {
            res.status(200).send({
              message: "Activation Email Successfully Sent (link will expire in 20 mins)!",
            });
          })
          .catch(err => {
            res.status(400).send({
...
// http://127.0.0.1:5000/login?activate=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InZhaWJoYXZzaW5naDI2MzNAZ21haWwuY29tIiwicGFzc3dvcmQiOiJwYXNzd29yZDEyMyIsInVzZXJuYW1lIjoidGVzdHVzZXIiLCJpYXQiOjE3MjAxMTkzOTIsImV4cCI6MTcyMDEyMDU5Mn0.GPJe6MMJP_KGMl-z_tCg2mq8QpY5hwvfYebcexXqetA
    ...
    if (err || !user) {
      ...
      return user.updateOne({ resetLinkToken: passwordResetToken }, (err, success) => {
        if (err) {
          ...
          // Send email for password reset using Resend
          resend.emails.send(msg)
            .then(() => {
              res.status(200).send({
                message: "Password Reset Email Successfully Sent (link will expire in 20 mins)!",
              });
            })
            .catch(err => {
              res.status(400).send({
...
router.get("/authenticate", (req, res) => {
  const activationToken = req.query.activate;
  if (activationToken) {
    jwt.verify(activationToken, config.ACTIVATION_SECRET, function (
      err,
      decodedToken
      ...
      UserModel.findOne(
        ...
        (err, existingUser) => {
          if (err) {
            ...
            if (existingUser) {
              return res.status(400).json({
                message: "This email or username has already been registered",
              });
            } else {
              const user = new UserModel({
                email,
